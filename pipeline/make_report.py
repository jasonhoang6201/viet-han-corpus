...
def main():
    ...
    cv = load(od / "char_validation.jsonl")
    ...
    st = Counter(c.get("status") for c in cv)
    n_q = sum(1 for c in cv if c.get("qn") == "?")
    n_low = sum(1 for r in rv if r["reason"] == "low_conf")
    n_red = sum(1 for r in rv if r["reason"] == "RED")
        ...
        if not args.no_crops and r.get("bbox"):
            ...
            if im is not None:
                try:
                    ...
                    crop = ""
        badge = "RED" if r["reason"] == "RED" else "low"
        bc = "#c0392b" if r["reason"] == "RED" else "#d98c00"
    ...
    verdict_rows = [
        ...
        ("Ký tự Hán (char)", len(cv), "OK", ""),
        ("OCR `?` (không đọc được)", f"{n_q} ({pct(n_q,len(cv))})", "OK" if n_q/max(len(cv),1) < .01 else "WARN", "đã vá bằng dict"),
        ("char BLACK/GREEN/RED", f"{st.get('BLACK',0)}/{st.get('GREEN',0)}/{st.get('RED',0)}", "OK", ""),
...
<div class="kpi">
  ...
  <div><b>{len(cv)}</b><span>ký tự Hán</span></div>
  <div><b>{pct(n_q,len(cv))}</b><span>OCR `?`</span></div>
...
<p><b>Phía Hán — TỐT, đủ nộp.</b> OCR 405 trang, {len(cv):,} ký tự, chỉ {n_q} ký tự `?`
({pct(n_q,len(cv))}) nhờ lớp từ điển KanjiDictVN. Trung vị độ tin cậy {statistics.median(confs):.3f}.
{st.get('BLACK',0):,} ký tự khớp từ điển (BLACK), chỉ {st.get('RED',0)} RED.</p>
...
({n_red} RED + {n_low} low-conf) đáng soát tay. Đó là &lt;{pct(n_low+n_red,len(hb))} số box.</p>
...
<li>Đối chiếu ảnh với cột <i>sinonom</i>. Nếu sai, ghi vào <code>review.jsonl</code> dòng tương ứng:
  ...
  <b>dict_gap</b> (đúng chữ nhưng thiếu âm Hán-Việt → điền âm), hoặc <b>drop</b> (rác, bỏ).</li>
<li>Chạy lại bước gộp để áp chỉnh sửa vào <code>han_boxes</code> / <code>char_validation</code> / <code>alignment</code>.</li>
</ol>
